...
public class CommandTpaccept extends CommandHandler<WarpPlugin> {

    public CommandTpaccept(WarpPlugin plugin) {
        super(plugin);
    }
        ...
        try {
            ...
            plugin.getUsers().fetch(player.getUniqueId()).thenAccept(user -> {
                try {
                    ...
                    plugin.getTeleporter().teleport(target, player);
    ...
    public List<String> complete(CommandSender sender, Command cmd, String label, String[] args) {
        ...
        User user = plugin.getUsers().get(player.getUniqueId());

        if (user == null)
            return Collections.emptyList();

        return user.getTeleportationRequests().stream()
